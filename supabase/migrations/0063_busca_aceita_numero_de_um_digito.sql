-- 0063 — A busca aceita "7"
--
-- A 0062 exige duas letras antes de procurar, e por um bom motivo: com uma
-- letra só, "a" devolveria a oficina inteira a cada tecla digitada.
--
-- Só que número de OS tem um dígito. A oficina que acabou de começar procura
-- pela OS 7 e não acha nada — e o teste ao vivo pegou isso na OS nº 1.
--
-- A regra passa a distinguir as duas coisas: texto continua precisando de duas
-- letras, e número de um dígito procura só entre as ordens, pelo número exato.
-- Assim "7" acha a OS 7 sem fazer o banco varrer motos e clientes atrás de um
-- "7" solto no meio de uma placa.
...
declare
  ...
  -- Curto demais para procurar por texto: só o número da ordem interessa.
  v_so_numero boolean := length(v_termo) < 2;
...
begin
  ...
  if v_termo = '' or (v_so_numero and v_numero is null) then
    return jsonb_build_object('clientes', v_clientes, 'motos', v_motos, 'ordens', v_ordens);
  end if;

  if not v_so_numero then
