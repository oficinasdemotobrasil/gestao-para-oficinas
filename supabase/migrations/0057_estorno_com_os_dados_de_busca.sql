-- 0057 — Achar a cobrança no Asaas sem sair da tela
--
-- A lista de estornos já dizia QUEM e QUANTO. Faltava o que resolve a parte
-- chata: encontrar a cobrança dentro do Asaas. A busca de lá aceita telefone,
-- e-mail e documento — os mesmos três dados que a assinatura mandou para lá no
-- dia em que o cliente foi criado.
--
-- Devolvê-los aqui é o que permite copiar e colar em vez de decorar. São dados
-- que a plataforma já tem; nada novo é coletado.
...
as $$
  ...
  from (
    select jsonb_build_object(
      ...
      /*
       * O que a busca do Asaas aceita. São exatamente os campos que a função
       * `assinatura` mandou para lá ao criar o cliente — então procurar por
       * qualquer um deles encontra a cobrança.
       *
       * O e-mail é o de quem assinou: o responsável pela oficina. Se houver
       * mais de um responsável, vale o mais antigo, que é quem estava lá no
       * dia da assinatura.
       */
      'telefone', o.telefone,
      'documento', o.cnpj,
    ...
    left join lateral (
      ...
      where u.oficina_id = o.id and u.perfil = 'admin'
